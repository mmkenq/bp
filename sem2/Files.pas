...
begin
  ...
  rewrite(StudentFile);
    with FlowStudent do
        while Ok do
        begin
          GoToXY(30,10); write('Будем еще вводить? (y/n)');
          ...
          begin
            GoToXY(30,20); writeln('Введите Имя студента');
            GoToXY(30,21); readln(Name);
            GoToXY(30,22); writeln('Введите оценку');
...
begin
 k:=3;
 write('Введите имя файла, который смотрим: ');
 readln(FileName);
 assign(StudentFile,FileName);
 reset(StudentFile);
 with FlowStudent do
...
var flag:integer;
    ...
    FlowStudent:Student;
begin
 writeln('Введите имя файла,в котором хотите найти отличников'); readln(FileName);
 assign(StudentFile,FileName); reset(StudentFile);
 ClrScr;
